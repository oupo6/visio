...
### 테스트 설계 두뇌 — *무엇을 테스트할지*를 두 비-자명 위험원으로 (측정)

좋은 판정·기억도 *틀린 테스트*를 굴리면 소용없다. 설계 두뇌는 *어떤 입력/상태에서 깨질지*를 두 가지
비-자명 위험원으로 끌어낸다(범용 모델을 엔진으로, 스캐폴드가 접지·기억·구조를 더함):
- **코드-이해(현재 구조):** 기능 구현을 읽어 *이 기능 고유의 위험표면*을 추출. 측정 — 코드에
  `split("<<RESET>>")[0]` 버그가 있으면 설명만 보는 설계는 그 임의 토큰을 절대 못 떠올리고(`0/3`),
  코드를 읽는 설계는 그 분기를 짚어 `<<RESET>>` 케이스를 스스로 만든다(`2/3`).
- **기억(과거 경험):** 위의 자율 학습 루프 — 과거에 깨진 약점을 회상해 재검사.

설계는 white-box(코드를 읽어도 됨)지만 *판정(verdict)은 여전히 오라클이 실제 상태로* 내린다 —
판정자 ≠ 빌더 독립성은 그대로다. (`verify_design.py`)
...
verify_design.py      # 설계 두뇌 — 코드-이해가 코드-특정 버그를 잡나
verify_*.py           # 판정 품질 벤치
...
### Test-design brain — deciding *what to test* from two non-obvious risk sources (measured)

A great judge and memory are wasted if you run the *wrong tests*. The design brain surfaces *where a
feature will break* from two non-obvious risk sources (the general model is the engine; the scaffold
adds grounding, memory, and structure):
- **Code understanding (current structure):** reads the implementation to extract *this feature's
  specific risk surface*. Measured — when the code has a `split("<<RESET>>")[0]` bug, description-only
  design never guesses that arbitrary token (`0/3`), while code-reading design spots the branch and
  writes the `<<RESET>>` case itself (`2/3`).
- **Memory (past experience):** the autonomous learning loop above — recall a past weak spot and re-test.

Design may be white-box (it can read the code), but the *verdict still comes from the oracle reading
real state* — judge ≠ builder independence holds. (`verify_design.py`)
...
verify_design.py      # design brain — does code-understanding catch code-specific bugs
